...
class ParseIDName:
    ...
    def parsePage(self, text_arg: str, type_arg: str):
        call_arg = {'track': 'tracks','artist': 'artists','album': 'albums','playlist': 'playlists'}
    ...
    def trackName(self) -> any:
        ...
        if data:
            ...
            if data_track_name:
                track_meta = []
                track_link = data.find_all('a', attrs={'class': ['d-track__title', 'deco-link', 'deco-link_stronger']}, text=f' {self.track_name} ')[0:8]
    ...
    def albumName(self):
        ...
        if data:
            data_album_name = data.find_all('a', attrs={'class': ['d-link', 'deco-link', 'album__caption']}, text=self.album_name)[:8]
            if data_album_name:
                album_meta = []
                for item in data_album_name:
                    album_id = re.findall(r'href=\"[\w\W]*\"', item.__str__())[0].split('href=')[1].split('"')[1].split('/')[2]
    ...
    def track_meta(self, id: Union[str, int]) -> dict:
        print("* track_meta")
        page = requests.get(f"https://music.yandex.ru/handlers/track.jsx?track={id}",  headers=self._HEADERS_JSX)
    ...
    def album_meta(self, id: Union[str, int]) -> dict:
        print("* album_meta")
        page = requests.get(f"https://music.yandex.ru/handlers/album.jsx?album={id}", headers=self._HEADERS_JSX)
        if page.status_code == 200:
            meta = {}
            data = page.json()
